...
# .ravel(): This method flattens the confusion matrix into a 1D array. So, the matrix [[tn, fp], [fn, tp]] becomes [tn, fp, fn, tp]. and returns it
...
# 1 The first index rows are used as the test set, and the remaining rows are used as the training set.
X_1_test = X.iloc[:index]
X_1_train = X.iloc[index:]
Y_1_test = Y.iloc[:index]
Y_1_train = Y.iloc[index:]

# 2 The second index rows are used as the test set.
# The training set includes rows from the start to the first index, and from the second index to the end.
# index:index*2= This specifies the range of rows to be selected. It means select rows starting from index up to but not including index*2.
# np.r_[:index, index*2:]= It creates a concatenation of row indices from 0 to index and from index*2 to the end.
#    [:index]= Selects rows from the start up to but not including index.
#    [index*2:]= Selects rows from index*2 to the end of the dataframe.
...
for i in range(0, K):
    # ----- RANDOM FOREST ----- #
    # rf_model: This is the Random Forest model instance.
    # fit(X_train_list[i], Y_train_list[i]): This method trains the model using the training data X_train_list[i] and the corresponding labels Y_train_list[i].
    # X_train_list[i] and Y_train_list[i] are the training data and labels for the i-th fold.
    rf_model.fit(X_train_list[i], Y_train_list[i])
    # predict(X_test_list[i]): This method uses the trained model to make predictions on the test data X_test_list[i].
    # rf_predictions: This variable stores the predictions made by the model.
    rf_predictions = rf_model.predict(X_test_list[i])
    # confusion_matrix(y_true=Y_test_list[i], y_pred=rf_predictions): This function computes the confusion matrix using the 
    # true labels Y_test_list[i] and the predicted labels rf_predictions.
